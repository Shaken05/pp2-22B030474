...
class Coin(pygame.sprite.Sprite):
    def __init__(self, enemy_pos):
        ...
        # Generate random weight for the coin
        weights = [1,2,3,4]  # Possible weights with different probabilities
        probabilities = [0.1,0.2,0.3,0.4]  # Probabilities for each weight
    ...
    def move(self):
        ...
        if (self.rect.top > SCREEN_HEIGHT):
            self.rect.top = 0
            self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
            # Regenerate weight for the coin
            weights = [1, 2, 3, 4]
...
while True:
    ...
    # Check if player collected enough coins to speed up the Enemy
